...
class Manager_Orders
{
	...
	*/
	...
	public function update_comments()
	{
		...
		if($this->ci->m_order->is_exists($order_id, $this->ci->manager_users->get_org_id())){
			if(is_array($added_comments)){
				foreach($added_comments as $comment){
					...
					$this->ci->m_order_comments->add_order_comment($order_id, $this->ci->manager_users->get_user_id(), $comment['text']);
				}
			}
