		...
		*/
		edit_field:function(options){
			...
			*/
			...
			var profile_col_input = $("<input type=\"text\" onkeypress=\"agent.profile.keypressed_edit_field({ jObjAct:$(this),name:'"+options.name+"',uri:'"+options.uri+"', type:'"+options.type+"'},event);\"  class=\"profile_col profile_col_input\"  />");
			var text = $.trim(profile_col_text.text());
			profile_col_input.val(text);
		    profile_col_text.replaceWith(profile_col_input); 
		
			profile_col_action.replaceWith("<span class=\"profile_col profile_col_action\"><a href=\"#\" onclick=\"agent.profile.click_edit_field({ jObjAct:$(this),name:'"+options.name+"', uri:'"+options.uri+"', type:'"+options.type+"' });return false;\"> Сохранить </a> </span>")
		},
		...
		show_field:function(options){
			...
			*/
			...
			$.ajax({
					...
					if(response.code && response.data){
						...
						switch (response.code){
							case "success_edit_profile":
								...
								profile_col_action.replaceWith("<span class=\"profile_col profile_col_action\"> <a href=\"#\" onclick=\"agent.profile.click_show_field({jObjAct:$(this),uri:'"+options.uri+"',name:'"+options.name+"', type:'"+options.type+"'});return false;\">Изменить</a></span>"); 	
				...
				beforeSend:function(){
					options.jField.append('<span class="profile_col preloader"> <img src="'+agent.baseUrl+'themes/dashboard/images/preloader.gif" /> </span>')
				},
				complete:function(){
